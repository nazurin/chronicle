			...
			# 鑑賞履歴（作品ページ内）
				...
				while(my $v = $rireki_syutoku->fetchrow_hashref){
					print "<div class='gyou' data-rireki='${\( sub { return $v->{id} if defined $v->{id} }->() )}'>";
						...
						print "</div>";
						print "<div class='sintyoku'>$v->{part}／$v->{whole}${\($josuu_tati_tekilang->{$v->{josuu}}{$Kahifu::Junbi::lang} or '')}</div>";
...
if($paginate == 1){
	...
	while(my $v = $koyomi_hantyuu->fetchrow_hashref){
		...
		my @hantyuu_inner_array = !defined $kensaku ? ($v->{week}, $hantyuukei_atumari->{"$hantyuukei_syori"}{winner}{hantyuu}, $hantyuukei_atumari->{"$hantyuukei_syori"}{winner}{count}, $hantyuukei_syori, $hantyuukei_atumari->{$hantyuukei_syori}, $hantyuukei_atumari->{"$hantyuukei_syori"}{total}) :
				...
				while(my $v = $soutei_rireki->fetchrow_hashref){
					print "<div class='gyou type_$v->{stop}'>";
						...
						print "<div class='sinkou'>";
							...
							print "<span><input type='text' name='whole' value='' placeholder='${\(defined $v->{whole} ? $v->{whole} : undef)}'></span>";
						print "</div>";
						...
						print "<div class='dainigyou'>";
							...
							print "<div class='memo'><span>memo</span><input type='text' name='memo' value='' placeholder='${\(defined $v->{memo} ? $v->{memo} : undef)}'></div>";
							print "</div>";
						print "</div>";
		...
		if(((not defined $v->{kakusu}) || $v->{kakusu}!=1) || Kahifu::Template::tenmei()){
			print "<div class='koumoku type_$v->{hantyuu}${\( sub { return ' hankakusi' if defined $v->{kakusu} && $v->{kakusu}==1 }->() )}'>";
				...
				print "<div id='kk_$v->{id}' class='kansou_kousin'>";
					...
					print "<div class='cour'><div><div>${\(Kahifu::Template::dict('KIROKU_COLLECTION_ALT'))}</div><div><input type='text' size='10' name='kaisi' placeholder='${\( sub { return $v->{kaisi} if defined $v->{kaisi}}->() ) }' value='${\( sub { return $v->{kaisi} if defined $v->{kaisi}}->() ) }'><input type='text' size='10' name='syuuryou' placeholder='${\( sub { return $v->{syuuryou} if defined $v->{syuuryou}}->() ) }' value='${\( sub { return $v->{syuuryou} if defined $v->{syuuryou}}->() ) }'></div><div><input type='text' size='6' name='cour' placeholder='${\( sub { return $v->{cour} if defined $v->{cour}}->() ) }' value='${\( sub { return $v->{cour} if defined $v->{cour}}->() ) }'></div></div></div>";
