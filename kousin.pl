...
### 初期関数設定ずみ ###############
	...
	sub update_backtrace_part_error {
		...
		my $sai_kakunin = $jyoukyou eq '再' || $jyoukyou eq '没' ? 1 : 0;
		Hyouka::External::mal_kousin($info->{$passthrough_id}{mal_id}, $part, $josuu, $jyoukyou, $sai_kakunin, $part_turu[1], $info->{$passthrough_id}{hantyuu}, $api_json_data) if (grep{$_ eq $info->{$passthrough_id}{hantyuu}} 13, 14, 17) && defined $info->{$passthrough_id}{mal_id} && $info->{$passthrough_id}{mal_id} ne '';
		Hyouka::External::al_kousin($info->{$passthrough_id}{al_id}, $part, $josuu, $jyoukyou, $sai_kakunin, $owari, $part_turu[1], $api_json_data) if (grep{$_ eq $info->{$passthrough_id}{hantyuu}} 13, 14, 17) && defined $info->{$passthrough_id}{al_id} && $info->{$passthrough_id}{al_id} ne '';
	}
	...
	if(not defined param('sakujyo')){
		# 削除ではない場合
		if(($info->{$passthrough_id}{part} < $part || ($info->{$passthrough_id}{part} != $part && $info->{$passthrough_id}{josuu} ne $josuu) || ($info->{$passthrough_id}{part} == 0 && $info->{$passthrough_id}{count} == 0) || ($info->{$passthrough_id}{part} == $part && $info->{$passthrough_id}{text} ne decode_utf8(param('title'))) || ($info->{$passthrough_id}{eternal} == 1) || $info->{$passthrough_id}{sakujyoukyou} eq '落' && $info->{$passthrough_id}{part} >= $part) && $part <= param('whole') && not (grep{$_ eq $info->{$passthrough_id}{sakujyoukyou}} '終', '再', '没') && not (grep{$_ eq param('mode')} 4, 5)){
